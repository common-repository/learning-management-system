...
class UserVerification {
	...
	private static function init_hooks() {
		...
		add_filter( 'wp_authenticate_user', array( __CLASS__, 'masteriyo_custom_session_limit_check_filter' ), 10, 2 );
	}
	...
	public static function masteriyo_custom_authentication_filter( $user ) {
		...
		if ( ! is_wp_error( $masteriyo_user ) && UserStatus::SPAM === $masteriyo_user->get_status() ) {
			$resend_verification_link = '<a href="' . masteriyo_generate_resend_verification_link( $masteriyo_user->get_id() ) . '">' . __( 'Resend verification email', 'learning-management-system' ) . '</a>';
			/* translators: %s: Resend verification link. */
			$error_message = sprintf( __( 'Please verify your email to log in. %s', 'learning-management-system' ), $resend_verification_link );
	 ...
	 * Filter callback for custom authentication in Masteriyo plugin.
	 *
	 * Checks users session limit before allowing authentication.
	 *
	 * @since 1.9.3
	 *
	 * @param WP_User|mixed $user The user object or original input.

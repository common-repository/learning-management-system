...
class LMSMigrationController extends RestController {
	...
	private function migrate_lp_data( $type, $remaining_ids ) {
		...
		// Delete remaining migrated items after all items have been migrated.
	...
	private function migrate_lp_courses( $remaining_ids ) {
		...
		$response = $this->generate_migration_response_from_lp( $lp_course, $remaining_courses );
	 ...
	 */
	private function generate_migration_response_from_lp( $lp_course, $remaining_courses ) {
		$type          = 'courses';
	...
	private function migrate_ld_courses() {
		$ld_courses = $this->get_learndash_courses();
		...
		$response = $this->generate_migration_response_from_ld( $ld_course, $remaining_courses );
	 ...
	 * @return array Migration response.
	 */
	private function generate_migration_response_from_ld( $ld_course, $remaining_courses ) {
		global $wpdb;
	...
	public function migrate_ld_course( $course_id ) {
		...
		if ( empty( $total_data['sfwd-lessons'] ) && ! empty( $total_data['sfwd-quiz'] ) ) {
			$author_id  = get_post_field( 'post_author', $course_id );
			$section_id = $this->insert_post( __( 'Section', 'learning-management-system' ), '', $author_id, PostType::SECTION, 0, $course_id );
			update_post_meta( $section_id, '_course_id', $course_id );
	...
	private function migrate_ld_orders() {
		...
		$ld_orders = $this->get_learndash_orders();
	 ...
	 */
	private function get_learndash_orders() {
		global $wpdb;
